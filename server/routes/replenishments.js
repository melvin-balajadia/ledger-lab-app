...
router.post('/:id/replenishments', async (req, res, next) => {
  ...
  try {
    ...
    await conn.commit();
    const { rows } = await pool.query('SELECT * FROM replenishments WHERE id = ANY(?)', [insertedIds]);
...
router.post('/:id/replenishments/:repId/restore', async (req, res, next) => {
  ...
  try {
    ...
    await conn.commit();
    const { rows: restored } = await pool.query('SELECT * FROM replenishments WHERE id = ?', [req.params.repId]);
    res.json(restored[0]);
  } catch (err) {
    await conn.rollback();
    const mapped = mapDbError(err);
